...
test.describe('authentication and authorisation', () => {
  ...
  test('the landing page renders and links to the required sitemap', async ({ page }) => {
    await page.goto('/');

    await expect(page.getByRole('heading', { level: 1 })).toContainText('housing society');
    await page.getByRole('link', { name: 'See the sitemap' }).first().click();

    await expect(page).toHaveURL(/\/sitemap$/);
    await expect(page.getByRole('heading', { name: 'Application sitemap' })).toBeVisible();
